...
export default {
		...
		let registries = await Promise.all(
			...
			listings
				.filter(
					item =>
						item.listing !==
						'QmWqzxG4aff4htBi4sEeQiK42L8oPTRE6cbk44HRxgJvLZ',
				)
	...
	getRegistryLocalization: async registry => {
		return registriesCache.has(registry)
			? registriesCache.get(registry).localization
			: ''
	},
	...
	getListingsToClaimReward: async (registry, account) => {
		let listings = await contractService.getListingsToClaimReward(
			registry,
			account,
		)
		return listings
	...
	getParameterizerProposals: async (registry, account) => {
		console.log(
			'getParameterizerProposals in backendless api',
			registry,
			account,
		)
		// let parameterizerProposals = await contractService.getParameterizerProposals(
		// 	registry,
		// 	account,
		// )
